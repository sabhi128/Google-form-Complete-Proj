...
export default function Forms({ forms, setForms }) {
  ...
  const saveFormToBackend = async (formData) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to save form: ${res.status} - ${errorText}`);
      }
  ...
  const handleCopyComponent = (id) => {
    ...
    if (componentToCopy) {
      ...
      saveFormToBackend(newComponent); // Save to backend
    }
  };

  const handleDeleteComponent = (id) => {
    setForms((prev) => prev.filter((comp) => comp.id !== id));
    // Optionally, send a delete request to backend if you implement it
  };
